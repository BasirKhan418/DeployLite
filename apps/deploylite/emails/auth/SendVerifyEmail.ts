...
const SendVeriyEmail = async(email: string, token: string,name:string) => {
  console.log('email is for sending email',email)
try{
    const transporter = await nodemailer.createTransport({
      ...
      const info = await transporter.sendMail({
        ...
        subject: `Complete Your Sign-Up – Verify Your Email Now `, // Subject line
...
<div style="margin:0px auto;max-width:640px;background:transparent;"><table role="presentation" cellpadding="0" cellspacing="0" style="font-size:0px;width:100%;background:transparent;" align="center" border="0"><tbody><tr><td style="text-align:center;vertical-align:top;direction:ltr;font-size:0px;padding:40px 0px;"><!--[if mso | IE]>
      ...
      <![endif]--><div style="cursor:auto;color:white;font-family:Whitney, Helvetica Neue, Helvetica, Arial, Lucida Grande, sans-serif;font-size:36px;font-weight:600;line-height:36px;text-align:center;">Welcome to DeployLite!</div><!--[if mso | IE]>
      </td></tr></table>
...
<p>Wowwee! Thanks for registering an account with DeployLite! You're the coolest person in all the land (and I've met a lot of really cool people).</p>
<p>Before we get started, we'll need to verify your email.</p>

          </div></td></tr><tr><td style="word-break:break-word;font-size:0px;padding:10px 25px;" align="center"><table role="presentation" cellpadding="0" cellspacing="0" style="border-collapse:separate;" align="center" border="0"><tbody><tr><td style="border:none;border-radius:3px;color:white;cursor:auto;padding:15px 19px;" align="center" valign="middle" bgcolor="#7289DA"><a href="${process.env.NEXT_URL}/api/auth/verify?token=${token}" style="text-decoration:none;line-height:100%;background:#7289DA;color:white;font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:15px;font-weight:normal;text-transform:none;margin:0px;" target="_blank">
            Verify Email
      ...
      In DeployLite, if you try to deploy an outdated codebase, the platform will humorously suggest a "code revival" with a witty message, encouraging you to pull the latest changes before continuing.
      ...
      Sent by DeployLite• <a href="https://deploylite.tech/blogs" style="color:#1EB0F4;text-decoration:none;" target="_blank">check our blog</a> • <a href="https://twitter.com/discordapp" style="color:#1EB0F4;text-decoration:none;" target="_blank">@deploylite</a>
